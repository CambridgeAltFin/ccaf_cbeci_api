...
def data(value):
    ...
    return jsonify(data=[DataResource(timestamp, row) for timestamp, row in energy_consumption_service.get_data(float(value))])
...
def monthly_data(value):
    energy_consumption_service = EnergyConsumptionServiceFactory.create()
    return jsonify(data=[MonthlyDataResource(date, row) for date, row in energy_consumption_service.get_monthly_data(float(value))])
