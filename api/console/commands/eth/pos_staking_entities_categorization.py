    ...
    with psycopg2.connect(**config['custom_data']) as connection:
        ...
        try:
            ...
            delete_sql = f"DELETE FROM {table_name} " \
                f"WHERE timestamp = %s AND category = %s"
            insert_sql = f"INSERT INTO {table_name} (category, node_count, timestamp) " \
                f"VALUES %s"
            ...
            for index, row in data.iterrows():
                cur_date = calendar.timegm(datetime.datetime.strptime(
                ...
                if (prev_date != cur_date and not (prev_date is None)):
                    for hosting_type in day_data:
                        total = sum(day_data[hosting_type].values())
                        insert_data.append(
                            (hosting_type, total, prev_date))
                        delete_data.append((prev_date, hosting_type))
                    day_data = {}
