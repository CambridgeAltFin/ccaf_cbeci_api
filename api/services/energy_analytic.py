...
class EnergyAnalytic(object):
    ...
    def get_data(self, price: float):
        metrics = ['min_consumption', 'max_consumption', 'guess_consumption', 'min_power', 'max_power', 'guess_power']

        return self._get_energy_dataframe(price, metrics).iterrows()
    ...
    def get_date_metrics(self, price: float, timestamp: int, prof_threshold_value: float, metrics: List) -> Union[Dict[str, float], None]:
        hash_rates = {}
    ...
    @staticmethod
    def smooth_data(data: List[Union[Dict[str, float], None]]) -> List[Dict[str, float]]:
        smooth_data = []
